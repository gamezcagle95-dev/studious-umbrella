...
def execute_agentic_loop():
    ...
    if not run_process(["/bin/bash", "bin/build.contracts.sh"], "Compiling Solidity contracts via build script"):
        sys.exit(1)
    ...
    if not run_process(integration_cmd, "Executing end-to-end cryptographic and token settlement test"):
        sys.exit(1)
    ...
    try:
        ...
        commit_msg = "feat: integrate EIP-712 DataAssetRegistry with off-chain appraisal engine"
