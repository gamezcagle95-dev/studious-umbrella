...
def run_integration_test():
    ...
    # 5. Local Cryptographic Assertions
    eip712_payload = {
        "types": {
            "EIP712Domain": [
    ...
    recovered_address = Account.recover_message(
        encode_typed_data(full_message=eip712_payload),
        signature=signature
    )
    ...
    # 6. Circuit Breaker Math Verification
    print("[Wurk] Verifying Circuit Breaker math...")
    estimated_tokens = 1000
    max_price_per_token = 100 * 10**18 # High limit
    price_per_token = price_in_eit // estimated_tokens
    print(f"[Wurk] Price per token: {price_per_token} EIT base units")
    assert price_per_token < max_price_per_token, "Circuit breaker math failed!"

    # Simulate a "trip"
    low_max_price = 1 # 1 wei per token
    assert price_per_token > low_max_price, "Circuit breaker should have tripped!"
    print("[Wurk] Circuit breaker logic verified (math alignment).")
