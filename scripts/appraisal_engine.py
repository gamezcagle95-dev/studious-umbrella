...
class AppraisalParams:
    """Encapsulates EIP-712 appraisal signing parameters for Pylint compliance."""
...
def sign_appraisal_eip712(params: AppraisalParams) -> str:
    ...
    structured_msg = encode_typed_data(full_message=eip712_payload)
    account = Account.from_key(params.private_key) # pylint: disable=no-value-for-parameter
    signed_msg = account.sign_message(structured_msg)
