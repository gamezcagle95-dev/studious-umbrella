...
contract DataAssetRegistry is AccessControl, EIP712 {
    ...
    function purchaseAsset(
        ...
        if (estimatedTokens > 0) {
            // Corrected Math: price is already in 18-decimal base units
            uint256 pricePerToken = appraisal.price / estimatedTokens;
            if (pricePerToken > maxPricePerTokenInEIT) {
                // NOTE: We emit the event before reverting to help off-chain monitoring.
                // However, since we revert, isPaused will NOT persist.
                // Protocol safeguards require an admin to pause based on this signal.
                emit CircuitBreakerTriggered(appraisal.assetHash, appraisal.price, estimatedTokens);
                revert("Anomaly Detected: Price-per-token exceeds limit.");
            }
        }
        ...
        // 6. Cross-Contract Call to Mint Data NFT License
